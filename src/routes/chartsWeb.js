...
RoutChartsWeb.get('/num-visitas-reales-main', (req, res) =>{
    req.getConnection((err, conn) => {
        ...
        conn.query(`SELECT VIS_ID, CLI_ID, CLI_SAP, VIS_ESPECIALISTA, EMP_ID, EMP_NOMBRE, EMP_APELLIDO, EMP_EQUIPO, EMP_ZONA, EMP_CIUDAD, EMP_CARGO, EQU_ID, EQU_EQUIPO, EQU_LIDER, VIS_CLIENTE, VIS_MOTIVO_CONTACTO, VIS_HORA_INICIO, VIS_HORA_FIN, VIS_FECHA, REA_FECHA, REA_HORA, VIS_OBSERVACION, CLI_NOMBRE, MOT_MOTIVO, MOT_ID, REA_RESULTADO  FROM visita INNER JOIN real_visita ON REA_VIS = VIS_ID INNER JOIN cliente ON VIS_CLIENTE = CLI_ID INNER JOIN motivo_contacto ON MOT_ID = VIS_MOTIVO_CONTACTO INNER JOIN empleado ON EMP_ID = VIS_ESPECIALISTA INNER JOIN equipo ON EMP_EQUIPO = EQU_ID WHERE VIS_REAL = 1 AND VIS_CANCELADO = 0 AND EMP_ACTIVO = 1`,(err,rows) => {
            if(err) return res.send(err)
