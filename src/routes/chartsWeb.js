...
RoutChartsWeb.get('/visitas-main', (req, res) =>{
    req.getConnection((err, conn) => {
        ...
        conn.query(`SELECT VIS_ID, CIU_CIUDAD, ZON_ZONA, CLI_ID, CLI_SAP, CLI_NOMBRE, CLI_NIT, VIS_ESPECIALISTA, EMP_ID, EMP_NOMBRE, EMP_APELLIDO, EMP_EQUIPO, EMP_ZONA, EMP_CIUDAD, EMP_CARGO, EMP_CEDULA, EQU_ID, EQU_EQUIPO, EQU_LIDER, VIS_CLIENTE, VIS_MOTIVO_CONTACTO, VIS_HORA_INICIO, VIS_HORA_FIN, VIS_FECHA, VIS_REAL, VIS_CANCELADO, VIS_OBSERVACION, VIS_CLASE_CONTACTO, VIS_OBSERVACION, VIS_FECHA_CREACION, CLI_NOMBRE, MOT_MOTIVO, MOT_ID FROM visita INNER JOIN cliente ON VIS_CLIENTE = CLI_ID INNER JOIN motivo_contacto ON MOT_ID = VIS_MOTIVO_CONTACTO INNER JOIN empleado ON EMP_ID = VIS_ESPECIALISTA INNER JOIN equipo ON EMP_EQUIPO = EQU_ID INNER JOIN ciudad ON EMP_CIUDAD = CIU_ID INNER JOIN zona ON ZON_ID = EMP_ZONA WHERE EMP_ACTIVO = 1 ORDER BY VIS_ID DESC`,(err,rows) => {
            if(err) return res.send(err)
...
//-------------------------------------------------------------------------|MAIN Visitas reales con resultado|
RoutChartsWeb.get('/visitas-resultados-main', (req, res) =>{
    req.getConnection((err, conn) => {
        if(err) return res.send(err)

        conn.query(`SELECT RES_RAZON, SEG_VISITA, SEG_ID, SEG_RESULTADO , SEG_RESULTADO_OTRO, SEG_RAZON, SEG_RAZON_OTRO, SEG_OBSERVACION, RES_RESULTADO, RES_RESULTADO, RES_TIPO, VIS_ID, CLI_NIT, CLI_ID, CLI_SAP, VIS_ESPECIALISTA, EMP_ID, EMP_NOMBRE, EMP_APELLIDO, EMP_EQUIPO, EMP_ZONA, EMP_CIUDAD, EMP_CARGO, EQU_ID, EQU_EQUIPO, EQU_LIDER, VIS_CLIENTE, VIS_MOTIVO_CONTACTO, VIS_HORA_INICIO, VIS_HORA_FIN, VIS_FECHA, VIS_REAL, VIS_CANCELADO, VIS_OBSERVACION, VIS_CLASE_CONTACTO, REA_FECHA, REA_HORA, REA_DIRECTA, VIS_OBSERVACION, VIS_FECHA_CREACION, CLI_NOMBRE, MOT_MOTIVO, MOT_ID, REA_RESULTADO  FROM visita INNER JOIN real_visita ON REA_VIS = VIS_ID INNER JOIN cliente ON VIS_CLIENTE = CLI_ID INNER JOIN motivo_contacto ON MOT_ID = VIS_MOTIVO_CONTACTO INNER JOIN empleado ON EMP_ID = VIS_ESPECIALISTA INNER JOIN equipo ON EMP_EQUIPO = EQU_ID INNER JOIN seguimiento ON VIS_ID = SEG_VISITA INNER JOIN resultado ON RES_ID = SEG_RESULTADO INNER JOIN resultado_razon ON RES_RAZ_ID = SEG_RAZON WHERE EMP_ACTIVO = 1`,(err,rows) => {
            if(err) return res.send(err)
