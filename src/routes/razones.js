...
const RoutRazones = express.Router()

RoutRazones.get('/list/:ID', (req, res) =>{
...
RoutRazones.get('/', (req, res) =>{
    req.getConnection((err, conn) => {
        if(err) return res.send(err)

        conn.query('SELECT * FROM resultado_razon WHERE RES_RAZON != "OTRO"',(err,rows) => {
            if(err) return res.send(err)
